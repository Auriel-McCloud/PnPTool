...
- **Korrektur nach Feedback ("ganz hochscrollen ist unpraktisch")**: erst als EIN globaler Schalter oben im `CharacterSheetPanel` gebaut, der sowohl Trait-Max-Buttons als auch Gegenstands-Meta steuerte — musste dafür immer ganz nach oben gescrollt werden. Jetzt zwei unabhängige, lokal platzierte Schalter: `showTraitOptions` direkt über der Werte-Liste, `showItemOptions` direkt über der Gegenstände-Liste. `GegenstandRow` bekommt weiterhin eine `showOptions`-Prop, die aber jetzt `showItemOptions` ist statt eines geteilten globalen States.
