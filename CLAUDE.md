...
- ✅ **Phase 2b** — Cytoscape-Graph-Ansicht: `GET /api/campaigns/{cid}/graph?focus=&depth=` liefert Cytoscape-JSON (voller Graph oder BFS-begrenzte Nachbarschaft), Frontend zeigt Personen/Orte/Events als farbige Knoten, Klick fokussiert auf Nachbarschaft
- ⏳ **Gerade in Arbeit / frisch gefixt**: Cytoscape-Container-Sizing-Bug (siehe unten) — **letzter Schritt war ein Fix, der noch nicht vom Nutzer bestätigt wurde.** Nächster Schritt: prüfen ob der Graph nach hartem Browser-Reload stabil steht (kein Wachsen mehr, kein Klick-Versatz mehr)
...
5. **Cytoscape Initial-Messung zu früh**: Falls der Container beim `cytoscape({container:...})`-Aufruf noch nicht final gelayoutet ist (z.B. direkt nach Tab-Wechsel), kann die interne Erstmessung falsch/zu klein ausfallen und bleibt es auch (Canvas nur "halb" befüllt, Klick-Koordinaten verschoben). Fix: `requestAnimationFrame` nach Konstruktion, das explizit `cy.resize()` + `cy.fit()` erzwingt, nachdem der Browser das Layout tatsächlich committed hat.
...
1. Nutzer testet den Graph-Sizing-Fix (siehe oben) — bei Erfolg: Phase 2 als komplett abhaken
2. UI-Polish-Durchgang (größere Formularfelder, fehlende Notizen-Felder ergänzen) — auf Wunsch des Nutzers zurückgestellt, noch offen
3. Granulare Sichtbarkeit (pro Feld, pro Spieler) designen — vor Phase 4 nötig
4. Danach: Phase 3 (Spieler-Charakterbogen)
