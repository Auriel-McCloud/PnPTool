...
- `Gegenstand` hat jetzt: `typ` (freier String, Frontend schlägt per `<datalist>` Allgemein/Waffe/Rüstung/Droge/Werkzeug/MacGuffin vor — bewusst kein starres Enum, damit neue Typen kein Backend-Update brauchen), `eigenschaften` (freie Key-Value-Paare wie Munition/Schaden/Preis/Level, als JSON-String in Neo4j gespeichert weil Neo4j keine Map-Properties unterstützt — De/Encoding passiert in `items/repository.py`, Schema/Routes sehen ein normales `dict[str,str]`), `zeigeInGraph` (bool), `bildUrl`.
