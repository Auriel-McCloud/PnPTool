...
- 🟢 **Phase 5** — **SL-Popups live über WebSocket gebaut** (eigener Abschnitt unten). **Messenger-Frontend fertig** (Persona-5-Stil, Neon-Cyberpunk): `frontend/src/kontakte/Messenger.tsx` mit schrägen Sprechblasen, animiertem Gitter-Hintergrund und Kontakt-Karten. Backend (Kontakte, Nachrichten) war bereits fertig. **Messenger-Persistenz repariert (07.09.2026):** Nachrichten werden korrekt gespeichert und nach Reload im Verlauf angezeigt. **SL-Popup-Empfang ergänzt:** `MitteilungenBlitz` und `MitteilungPopup` jetzt auch im SL-Dashboard, damit die SL Chat-Benachrichtigungen (NACHRICHT-Art) von Spielern sieht. Offen: "Nur-Lesen"-Nachrichten für SL-Broadcasts.
...
**Nachgezogen (02.09.2026 — MagWare, Körperzonen-Slots, Item-Trait-Boni, Ausrüstungsfertigkeiten):**

Ausgangspunkt: Marks Idee für eine dritte Chrom-Flavor-Kategorie (MagWare, magische Implantate), echte Platzbelegung in den Körperzonen ("wo sitzt es"), ein Sandkasten zum Bauen von Cyber-/Bio-/MagWare, und Gegenstände, die eine Fertigkeit *verleihen* statt nur Schaden zu machen (Beispielidee "Zauberstab mit Feuerball 3", später präzisiert zu "gibt Bonus auf einen bestehenden Wert ODER erzeugt eine ganz neue Fertigkeit, wie ein Cyberdeck es mit NeuroWeaving-Skills schon tut").

- **MagWare**: dritter Eintrag in `CHROM_TYPEN` (Backend `items/repository.py`) und `TYP_OPTIONEN` (Frontend). **Keine eigene Kostenformel** — läuft über dieselbe WVerlust-Rechnung wie Cyber-/Bioware (Mark: "das wird durch den Willenskraftverlust geregelt"). Eine schematische Körper-Ansicht (Strichmännchen mit Augment-Markierungen) wäre ein hübsches Extra, aber vertagt — reiner Vision-Eintrag, siehe unten.
- **Slots**: `Gegenstand.slot` (1-3, `int | None`) neben dem bestehenden `koerperzone`-Feld. Backend prüft beim Ausrüsten (`POST .../ablage`) über `repository.slot_konflikt`, ob ein anderes ausgerüstetes Chrom-/Bio-/MagWare-Stück derselben Person Zone+Slot schon belegt — Konflikt liefert **409** mit dem Namen des blockierenden Stücks. Nur ausgerüstete Ware zählt; im Rucksack blockiert nichts. Slot ist optional (`None` = kein fester Platz), betrifft also nur, wer ihn explizit setzt.
- **Baukasten**: bewusst **kein** Komponenten-System — bleibt wie beim bisherigen Chrom-Editor (Stufe/Qualität antippen setzt Preis+WVerlust zugleich, siehe `chromstufen`-Endpoint). Gilt jetzt einheitlich für Cyber-/Bio-/MagWare.
- **`Gegenstand.istWaffe`** (bool): unabhängig vom Typ — eine Cyberklinge ist gleichzeitig Cyberware UND Waffe. Reine Kennzeichnung, keine eigene Mechanik.
- **`Gegenstand.traitBoni`** (`dict[str,int]`, als JSON-String gespeichert wie `fahrzeugFertigkeiten`): Bonuswürfel auf BESTEHENDE Attribute/Fertigkeiten/Sphären, Schlüssel ist der TraitDef-**Name** (nicht die ID — bleibt so über Rulesets hinweg lesbar). Wirkt nur solange ausgerüstet. Mehrere Gegenstände mit Bonus auf denselben Wert werden **summiert** (`repository.ausruestungs_trait_boni`) — anders als bei Cyberdecks, wo das Beste zählt, weil hier unterschiedliche Verbesserungen gemeint sind, keine austauschbaren Geräte.
- **`Gegenstand.ausruestungsfertigkeiten`** (`dict[str,int]`): NEUE Fertigkeiten, die es ohne den Gegenstand nicht gibt (z.B. "Springen 3" an magischen Stiefeln) — dasselbe Prinzip wie die Cyberdeck-B/S/D/K-Werte, nur frei benennbar statt fest verdrahtet. `repository.ausruestungsfertigkeiten_liste` sammelt sie über alle ausgerüsteten Gegenstände einer Person, bei Namenskollision zählt der höchste Bonus (mit Quellenangabe, welcher Gegenstand ihn gibt).
- **Charakterbogen zeigt beides** (Marks Wunsch "ich hätte gerne das 4. auch im Charakterbogen angezeigt"): `traits/routes.py::/bogen` liefert zusätzlich `ausruestungsBoni` und `ausruestungsfertigkeiten` neben dem bestehenden `deckBoni`. Frontend (`Charakterblatt.tsx`): normale Werte zeigen ein grünes `+N`-Badge neben dem Namen, wenn ein Ausrüstungsbonus greift (Bonus zählt auch für die Proben-Rechnung mit, `wert + ausruestungsBonus`). Ausrüstungsfertigkeiten bekommen einen eigenen Blattabschnitt ganz unten, mit dem Gegenstandsnamen als Quellenangabe statt eines Zahlen-Badges.
- **Editor** (`CharacterSheetPanel.tsx`, `GegenstandRow`): Körperzone+Platz-Dropdown und "zählt zusätzlich als Waffe"-Checkbox stecken im bestehenden Chrom-Bereich (nur bei Cyber-/Bio-/MagWare sichtbar). Trait-Boni und Ausrüstungsfertigkeiten stehen dagegen in einem **eigenen, typunabhängigen Abschnitt** direkt darunter — bewusst nicht auf Chrom beschränkt, weil Marks eigenes Beispiel (Zauberstab) eine Waffe/Sonstiges wäre, keine Cyberware. Trait-Boni werden über ein Dropdown aus dem echten TraitDef-Katalog gewählt (kein Freitext, um Tippfehler gegen den Katalog zu vermeiden); Ausrüstungsfertigkeiten sind dagegen Freitext, weil sie per Definition NICHT im Katalog stehen.
- **End-to-end verifiziert** (per curl gegen die Testkampagne): MagWare mit Slot+istWaffe+traitBoni+ausruestungsfertigkeiten angelegt, ausgerüstet → Willenskraftverlust am Bogen der Testfigur korrekt auf 1 gestiegen, `ausruestungsBoni` und `ausruestungsfertigkeiten` (mit Quellenangabe) korrekt im `/bogen`-Response. Zweiter Gegenstand am selben Platz → Ausrüsten liefert 409; anderer Platz → 200. Backend-Tests (139) und `npm run build` beide grün. Testgegenstände wieder gelöscht.
